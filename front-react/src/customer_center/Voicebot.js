...
const VoiceBot = () => {
  ...
  // WebSocket 연결 및 첫 인사
  useEffect(() => {
    const ws = new WebSocket("ws://3.39.177.144:8002/ws");
    ...
    ws.onopen = () => {
      ...
      // 첫 인사 음성 재생
      const welcomeAudio = new Audio("http://3.39.177.144:8002/static/welcome.mp3");
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === "text") {
        ...
        const audio = new Audio("http://3.39.177.144:8002" + data.audio_path);
        audio.play();
      }
    };
  ...
  const toggleRecording = async () => {
    if (!isRecording) {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        const res = await fetch("http://3.39.177.144:8002/upload-audio", {
          method: "POST",
          body: formData,
        });
        ...
        if (result.audio_path) {
          const audio = new Audio("http://3.39.177.144:8002" + result.audio_path);
          audio.play();
        }
        if (result.answer) {
