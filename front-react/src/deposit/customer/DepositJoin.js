...
const DepositJoin = () => {
  ...
  const [selectedWithdrawAccountNumber, setSelectedWithdrawAccount] =
    useState("");
    ...
    try {
      ...
      if (productIdFromQuery) {
        const matched = response.data.find(
          (p) => String(p.id) === productIdFromQuery
        );
        if (matched) {
  ...
  const fetchAccounts = async () => {
    try {
      const response = await RefreshToken.get(
        `/accounts/allAccount/${customerId}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !amount ||
      Number(amount.replace(/,/g, "")) < selectedProduct.minAmount
    ) {
      alert(
        `최소 ${
          selectedProduct.minAmount?.toLocaleString() ?? 0
        }원 이상 입금해야 합니다.`
      );
      return;
    }
    ...
    <div className="deposit-container">
        ...
        <>
          ...
          <div className="deposit-product-list">
            {products.map((product) => (
              <div
                key={product.id}
                className="deposit-product-card"
      ...
      {currentStep === "agreement" && selectedProduct && (
        <div className="deposit-card">
          ...
          <table className="deposit-agree-table">
            <tbody>
              ...
              <tr>
                <th>약관</th>
                <td>
                  본 상품은 예금자 보호법에 따라 보호됩니다. 금리 변동 등에
                  주의하시기 바랍니다.
                </td>
              </tr>
            </tbody>
          ...
          <div className="deposit-agree-buttons">
            <button className="deposit-btn-primary" onClick={handleAgree}>
              동의하고 가입하기
            </button>
            <button className="deposit-btn-cancel" onClick={handleCancel}>
              취소
            </button>
          </div>
        </div>
      ...
      {currentStep === "form" && selectedProduct && (
        <div className="deposit-card">
          ...
          <div
            className="deposit-selected-product"
          ...
          <form onSubmit={handleSubmit} className="deposit-form">
            <div className="deposit-flex-inputs">
              <div className="deposit-flex-column deposit-form-group">
                ...
                <div className="deposit-hint">
                  최소 입금액:{" "}
                  {selectedProduct.minAmount?.toLocaleString() ?? "0"}원
                </div>
              </div>
              <div className="deposit-flex-column deposit-form-group">
                <label>예금 계산기</label>
                ...
                {calcResult && (
                  <div className="deposit-calc-result">
                    <div>
                      총 이자 (세전):{" "}
                      {calcResult.interest?.toLocaleString() ?? "0"}원
                    </div>
                    <div>
                      세후 수령 이자:{" "}
                      {calcResult.afterTaxInterest?.toLocaleString() ?? "0"}원
                    </div>
                    <div>
                      총 수령액: {calcResult.total?.toLocaleString() ?? "0"}원
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="deposit-form-group">
              ...
              >
                ...
                {accounts.map((acc) => (
                  <option
                    key={acc.account_number || acc.accountNumber}
                    value={acc.account_number || acc.accountNumber}
                  >
                    {acc.account_name || acc.accountName || ""} (
                    {acc.account_number || acc.accountNumber}) - 잔액:{" "}
                    {acc.balance != null ? acc.balance.toLocaleString() : "0"}원
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="deposit-submit-buttons">
              <button type="submit" className="deposit-btn-primary2">
                가입 완료
              </button>
              <button
                type="button"
                className="deposit-btn-cancel"
