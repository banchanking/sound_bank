...
const SavingsJoin = () => {
  ...
  const [selectedWithdrawAccountNumber, setSelectedWithdrawAccountNumber] =
    useState("");
          ...
          <form onSubmit={handleSubmit} className="deposit-form">
              ...
              <div className="deposit-flex-column deposit-form-group">
                ...
                {selectedProduct && (
                  <div className="deposit-hint">
                    최소 {(selectedProduct.minAmount ?? 0).toLocaleString()}원
                    이상
                  </div>
                )}
              </div>
              <div className="deposit-flex-column deposit-form-group">
                ...
                {calcResult && (
                  <div className="deposit-calc-result">
                    <div>
                      총 납입금액:{" "}
                      {(calcResult.totalPrincipal ?? 0).toLocaleString()}원
                    </div>
                    <div>
                      세전 이자: {(calcResult.interest ?? 0).toLocaleString()}원
                    </div>
                    <div>
                      세후 이자:{" "}
                      {(calcResult.afterTaxInterest ?? 0).toLocaleString()}원
                    </div>
                    <div>
                      만기 수령액:{" "}
                      {(calcResult.maturityAmount ?? 0).toLocaleString()}원
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="deposit-form-group">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedWithdrawAccountNumber(e.target.value)
                }
              ...
              >
                ...
                {accounts.map((acc) => (
                  <option key={acc.account_number} value={acc.account_number}>
                    {acc.account_name} ({acc.account_number}) - 잔액:{" "}
