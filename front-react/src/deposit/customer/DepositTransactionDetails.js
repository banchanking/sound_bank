...
const DepositTransactionDetails = () => {
  ...
  const handleAccountChange = (e) => {
    ...
    const account = accounts.find(a => a.id === parseInt(id) && a.type.toUpperCase() === type.toUpperCase());
  ...
  const pagedTransactions = transactions.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  return (
    <div className="depositDtrans-container">
      <div className="depositDtrans-card">
        ...
        <div>
          <label>계좌 선택</label>
          <select onChange={handleAccountChange} value={selectedAccount ? `${selectedAccount.type}-${selectedAccount.id}` : ''}>
            <option value="">계좌를 선택하세요</option>
            {accounts.filter(acc => acc.accountStatus === 'ACTIVE').map(acc => (
        ...
        <div style={{ margin: "16px 0" }}>
          <button className="dpDayButton" onClick={() => setDateRangeByPreset('day')}>당일</button>
          <button className="dpDayButton" onClick={() => setDateRangeByPreset('week')}>1주일</button>
          <button className="dpDayButton" onClick={() => setDateRangeByPreset('month')}>1개월</button>
          <button className="dpDayButton" onClick={() => setDateRangeByPreset('6month')}>6개월</button>
        </div>
        ...
        <button className="depositDtrans-btn" onClick={handleSearch}>조회</button>
        ...
        {transactions.length > 0 && (
          <div className="pageButtonArea">
            {startPage > 1 && (
              <button className="pageArrow" onClick={() => setCurrentPage(startPage - 1)}>&lt;</button>
            )}
            {Array.from({ length: endPage - startPage + 1 }, (_, i) => startPage + i).map(num => (
              <button
                key={num}
                className={`pageButton ${currentPage === num ? 'activePage' : ''}`}
            ...
            {endPage < totalPages && (
              <button className="pageArrow" onClick={() => setCurrentPage(endPage + 1)}>&gt;</button>
            )}
          </div>
        )}
      </div>
